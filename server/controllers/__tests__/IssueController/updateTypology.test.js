...
describe('tests updateTypology method', () => {
  ...
  it('should successfully parse typology file and start/end transaction that creates each error', async () => {
    ...
    mockedIssueService.createIssue.mock.calls.forEach((mockCall) => {
      ...
      expect(mockedError.id).toEqual(mockCall[0]);
      expect(mockedError.parent).toEqual(mockCall[1]);
      expect(mockedError.name).toEqual(mockCall[2]);
      expect(mockedError.description).toEqual(mockCall[3]);
      expect(mockedError.notes).toEqual(mockCall[4]);
      expect(mockedError.examples).toEqual(mockCall[5]);
      expect(mockCall[6]).not.toEqual(undefined);
    });
  });
